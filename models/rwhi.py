...
class RWHIModule(BaseModule):
    ...
    预算膨胀 + 叠加策略: P_total = P_base + P_radar
    
    锚点预算分配 (v2.1 默认):
    - N_base = 900: 静态基础锚点 (匹配原始RaCFormer密度!)
    - N_radar = 300: 动态雷达锚点 (额外增益，聚焦高RCS区域)
    - N_total = 1200: 膨胀的总预算
    
    vs v2.0 (num_base=500):
    - v2.0 基础密度下降 44%，导致性能不如基线
    - v2.1 基础密度 = 基线，雷达增益是纯收益
    
    基础锚点分布 (P_base):
    - 近场 (2-15m): 密集采样 (~2m stride)，约55%的点
    - 中场 (15-30m): 中等密度 (~3m stride)，约30%的点
    ...
    def __init__(self,
                 num_query=1200,          # v2.1: 膨胀的总预算
                 num_base=900,            # v2.1: 匹配原始RaCFormer密度
                 num_radar=300,           # v2.1: 额外的雷达增益
                 embed_dims=256,
        ...
        if num_base + num_radar != num_query:
            print(f"[RWHI v2.1] Warning: num_base({num_base}) + num_radar({num_radar}) "
        ...
        print(f"[RWHI v2.1] Initialized: num_base={self.num_base}, num_radar={self.num_radar}, "
    ...
    def _precompute_base_anchors(self):
        ...
        # 打印分布统计
        print(f"[RWHI v2.1] Base anchors: {num_anchors} points, "
              f"rings={actual_num_rings} (near={near_rings}, mid={mid_rings}, far={far_rings}), "
              f"angles={num_angles}")
        print(f"[RWHI v2.1] Distance range: [{d.min().item()*r:.1f}m, {d.max().item()*r:.1f}m]")
